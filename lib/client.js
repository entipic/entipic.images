...
external.formatKey = function formatKey(size, id) {
	return util.format(KEY_FORMAT, id.substr(0, 4), size, id);
};
...
internal.save = function(client, id, body, sizes) {
	...
	return Promise.resolve(sizes).each(function(size) {
		return gm.resize(body, size.size, size.quality).then(function(resizeData) {
			return imagemin(resizeData).then(function(data) {
